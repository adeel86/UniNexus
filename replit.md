...
## Recent Changes
- **2026-04-11**: Completed role-alias production cleanup across backend access-control branches and middleware declarations by routing University/Industry/Admin checks through `hasRole()` canonical roles; fixed authenticated Autocomplete custom-entry/search requests; removed unused UI wrappers (`menubar`, `pagination`, `hover-card`, `context-menu`, `resizable`, `aspect-ratio`, `input-otp`); verified direct frontend `fetch()` calls are limited to auth/access-gate infrastructure.
- **2026-04-11**: Continued API route audit — verified teacher-content upload, unread count, follower/following, recruiter feedback insights, and university teacher routes against mounted backend routes; made recruiter feedback and My Teachers route-param query URLs explicit; fixed AI post suggestion refresh so it no longer appends the refresh counter as a path segment.
