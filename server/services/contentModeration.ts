...
export async function moderateImageUrl(imageUrl: string): Promise<ModerationResult> {
  ...
  try {
    ...
    if (!jsonMatch) return { flagged: false, reason: "", confidence: 0 };
...
export async function moderateText(text: string): Promise<ModerationResult> {
  ...
  try {
    ...
    if (!result) return { flagged: false, reason: "", confidence: 0 };
...
export async function moderatePostContent(params: {
  ...
  return { flagged: false, reason: "", confidence: maxConfidence };
}
