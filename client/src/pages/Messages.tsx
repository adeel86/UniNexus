          ...
          <ScrollArea className="flex-1">
            {connections.length === 0 && conversations.length === 0 ? (
              ...
              <div className="space-y-1">
                {/* Build a unified list: connections enriched with conversation data */}
                {(() => {
                  ...
                  const extraConvUsers: { connId: string; user: any }[] = conversations
                    .filter(conv => {
                      const other = conv.participants.find(p => p.id !== currentUser?.id);
                      return other && !connectionUserIds.has(other.id);
                    })
                    .map(conv => {
                      const other = conv.participants.find(p => p.id !== currentUser?.id)!;
                      return { connId: `conv-${conv.id}`, user: other };
                    });

                  const allUsers = [...connectionUsers, ...extraConvUsers];
                  ...
                  return allUsers.map(({ connId, user: connUser }) => {
                    const conversation = conversations.find(conv =>
                      conv.participants.some(p => p.id === connUser.id)
                    );
                    ...
                    const handleClick = () => {
                      if (conversation) {
                        ...
                        createConversation.mutate(connUser.id);
                      }
                    };
                    ...
                    return (
                      <div
                        ...
                        onClick={handleClick}
                        data-testid={`connection-chat-${connId}`}
                      >
